...
Markdownのリンクを作成するには、指定されたURLのwebページのタイトルを取得する必要がある。webページのアクセスするために利用するaxiosは非同期のため、実際に処理が終了する前に、その後のプログラムに進む。expectでテキストエリアの値を判定するときには、まだタイトルが取得できていないため、テストが失敗する。
